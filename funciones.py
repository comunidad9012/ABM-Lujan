...
def listarCursos(cursos):
    ...
    contador = 1
    for cur in cursos:
        datos = "{0}. Código: {1} | Nombre: {2} ({3} créditos)" #almacenados 
...
def pedirDatosRegistro():
    ...
    while(not codigoCorrecto):
        codigo = input("Ingrese código: ") #codigo de la materia
        if len(codigo) == 6:
            codigoCorrecto = True
        else:
            print("Código incorrecto: Debe tener 6 dígitos.") 

    nombre = input("Ingrese nombre: ") #nombre de la materia 

    creditosCorrecto = False
    while(not creditosCorrecto):
        creditos = input("Ingrese créditos: ") #credito de la materia
...
def pedirDatosEliminacion(cursos):
    listarCursos(cursos)
    existeCodigo = False
#ingresamos el codigo de la materia que queremos eliminar 
